...
export async function GET() {
  try {
    ...
    const debugInfo = teachers.map((t, index) => ({
      ...
      role: t.role,
      status: t.status,
      passwordLength: t.password ? t.password.length : 0,
      passwordFirstChar: t.password ? t.password.substring(0, 1) : '',
      passwordLastChar: t.password ? t.password.substring(t.password.length - 1) : ''
    }));
