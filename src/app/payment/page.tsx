...
// Removed Firebase imports - using MariaDB instead
// Removed unused Firebase type import
...
export default function BillingPage() {
    ...
    const proceedWithSelection = (plan: any) => {
        if (plan.id === 'enterprise') {
            ...
            router.push(`/payment/purchase?purchaseType=subscription&bundle=${plan.name} Subscription&credits=${plan.id}&price=${plan.priceGHS}`);
        }
    }
    ...
    const handleContact = async (method: 'sms' | 'email', smsMessage: string = enterpriseSmsBody) => {
        ...
        try {
            if (method === 'email') {
                // Replace Firebase with API call to save email request
                const response = await fetch('/api/communications', {
                    ...
                    body: JSON.stringify({
                        type: 'email',
                        recipient: 'nexorasystems25@gmail.com',
                        subject: "Request for Enterprise Demo & Pricing Details",
                        message: enterpriseEmailBody,
                        status: 'pending'
                    }),
                });
  ...
  destination: '0536282694',
  message: smsMessage,
  msgid: `enterprise-request-${Date.now()}`,
}], schoolInfo?.systemId || 'CHARIOT EDU');
                if (result.success) {
                    toast({
                        title: "Request Sent",
    ...
    if (isLoading) {
        return (
            <div className="space-y-6">
                <PageHeader
                    title="🚀 CampusFlow Cloud Subscriptions"
    ...
    return (
        <>
            ...
            <Card className="mt-12">
                <CardHeader>
                    <CardTitle>🔐 Why CampusFlow?</CardTitle>
                </CardHeader>
